...
export const maxSentencesForSentence = (entities: IEntities) => (sentence: ISentenceTokens[]) => {
    const sr = sentence.reduce((accumulator, t) => {
        ...
        if (t.type === 'Slot' || t.type === 'Alias') {
            ...
            if (t.opt) { innerEntityVariations++; }
...
export const maxSentencesForEntity = (
    ed: IChatitoEntityAST, entities: IEntities,
): number => ed.inner.map(maxSentencesForSentence(entities)).reduce((acc, val) => acc + val);
